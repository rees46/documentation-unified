...
:::warning Неполный контент
Статья в процессе переоформления.
:::
...
Платформа поддерживает возможность создания двух типов баннеров: графические и HTML. 

![Выбор баннера](./images/banners_html.png)
...
При создании html-баннера потребуется заполнить поля настроек.

Некоторые вам уже знакомы, но мы разберём все

1. **Содержимое баннера** - в это поле нужно вставить HTML разметку вашего баннера.
2. **Целевая ссылка** - определяет куда пользователь будет перенаправлен после клика на банер
3. **Диплинк iOS** - специальная ссылка, которая ведёт на конкретный экран мобильного приложения
4. **Диплинк Android** - то же самое для Android
5. **Теги** - для фильтрации баннеров на стороне клиента
6. **Позиция** - порядок возвращения баннеров в ответе API
7. **Статус** - баннер активен или не активен
8. **Erid** - поле для размещения рекламного Erid-токена
9. **Segment ids** - сегменты аудитории, которым будет показан баннер
10. **Exclude segment ids** - для выбранных сегментов аудитории данный баннер не будет показан
11. **Start date** - дата начала показа этого баннера
12. **Start time** - время начала показа этого баннера
13. **Finish date** - дата окончания показа
14. **Finish time** - время окончания показа
