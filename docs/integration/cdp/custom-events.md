...
Кастомные события можно вызывать как через SDK с использованием `did`, так и напрямую через API-запрос с использованием любого [идентификатора клиента](entities/identifier.md).
