...
| items        | Да          | Массив товаров для события. Может содержать как один товар, так и полное содержимое корзины. Массив не может быть пустым, если `full_wish` не true или отсутствует. При `full_wish`: `true` пустой массив `items` очищает избранное. |
