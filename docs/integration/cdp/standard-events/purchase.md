...
| items[].id       | Да          | Идентификатор (артикул) товара, который указан в импорте товара. Пустой ID или ID длиннее 255 символов будут проигнорированы (товар не добавится в покупку без  появления ошибки).                                                        |
| items[].quantity | Да          | Количество единиц этого товара в корзине клиента. По-умолчанию равно `1`. Отрицательные значения и `0` принудительно заменяются на `1`. Для весовых товаров всегда передавайте `1`, а полную стоимость указывайте в `price`.              |
| items[].price    | Да          | Стоимость одной единицы этого товара в заказе. Передавайте корректное положительное число. Отрицательные числа, строки и нечисловые значения будут проигнорированы (поле `price` будет отсутствовать в ответе, при этом ошибки не будет). |
