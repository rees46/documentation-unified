# API и SDK для листинга товаров

Эта статья описывает взаимодействие с SDK и REST API листинга товаров, запросы, структуру ответа и как отображать результаты для клиента. Эта статья не описывает, как выводить сайдбар с фильтрами для уточнения поиска. По этому вопросу ознакомьтесь со статьей про [фасетный поиск](./facet.md), которая является вторым обязательным шагом интеграции листинга товаров в категории.

```
GET https://api.rees46.ru/products
```
...
| sort_by            | String  | Нет         | Значение ключа, по которому сортировать результаты поиска: `popular`, `price`, `discount`, `sales_rate`, `date`, `price_margin`, `rating`                                                          |
| sort_dir           | String  | Нет         | Направление сортировки: `asc`, `desc`. По умолчанию: `desc`                                                                                                                                        |
...
| category_limit     | Integer | Нет         | Лимит значений категорий в фильтре по категории. По умолчанию `1000`. Чем выше число, тем дольше ответ поиска                                                                                      |
...
```shell [S2S]
# Запрос листинга товаров без фильтров (строится по всему товарному каталогу)
curl https://api.rees46.ru/products?shop_id=...&did=...&sid=...

# Фильтрация по бренду
curl https://api.rees46.ru/products?shop_id=...&did=...&sid=...&brands=BRAND_1,BRAND_2

# Фильтрация по категории товара
curl https://api.rees46.ru/products?shop_id=...&did=...&sid=...&categories=CATEGORY_1,CATEGORY_2

# Фильтрация по наличию товаров в локациях
curl https://api.rees46.ru/products?shop_id=...&did=...&sid=...&locations=LOCATION_1,LOCATION_2

# Фильтрация по мерчантам  (для маркетплейса)
curl https://api.rees46.ru/products?shop_id=...&did=...&sid=...&merchants=MERCHANT_1,MERCHANT_2

# Фильтр по списковым параметрам товаров
curl https://api.rees46.ru/products?shop_id=...&did=...&sid=...&filters={"PARAM_NAME_1": ["PARAM_VALUE_1", "PARAM_VALUE_2"], "PARAM_NAME_2": ["PARAM_VALUE_1", "PARAM_VALUE_2"]}

# Фильтр по булевым параметрам товаров
curl https://api.rees46.ru/products?shop_id=...&did=...&sid=...&filters={"PARAM_NAME_1": ["true"], "PARAM_NAME_2": ["false"]}

# С сортировкой по марже
curl https://api.rees46.ru/products?shop_id=...&did=...&sid=...&filters={"PARAM_NAME_1": ["true"], "PARAM_NAME_2": ["false"]}&sort_by=price_margin&sort_dir=desc
```
...
r46("products", {
  ...
  // handle the response: for example, render results
}, function(error) {
...
sdk.getProductsList(
    ...
    switch result {
    case .success(let response):   // response: ProductsListResponse
        print(response)
    case .failure(let error):      // error: SdkError
        print(error)
    }
}
```
  ...
  "products": [
    {
      ...
      "url": "https://demo.r46.dev/product/chaynik-tefal-thermo-protect-ko-140ae0-286429?recommended_by=listing",
      ...
      "categories": [
        {
          ...
          "url": "https://demo.r46.dev/category/tehnika-dlja-kuhni?recommended_by=listing",
          "name": "Техника для кухни",
          "level": "1",
          "name_with_parent": "Техника для кухни",
          "url_handle": "/category/tehnika-dlja-kuhni?recommended_by=listing"
        },
        {
          "id": "prigotovlenie-napitkov",
          "parent_id": "tehnika-dlja-kuhni",
          "url": "https://demo.r46.dev/category/tehnika-dlja-kuhni/prigotovlenie-napitkov?recommended_by=listing",
          "name": "Приготовление напитков",
          "level": "2",
          "name_with_parent": "Техника для кухни - Приготовление напитков",
          "url_handle": "/category/tehnika-dlja-kuhni/prigotovlenie-napitkov?recommended_by=listing"
        },
        {
          "id": "jelektricheskie-chajniki",
          "parent_id": "prigotovlenie-napitkov",
          "url": "https://demo.r46.dev/category/tehnika-dlja-kuhni/prigotovlenie-napitkov/jelektricheskie-chajniki?recommended_by=listing",
          "name": "Электрические чайники",
          "level": "3",
          "name_with_parent": "Приготовление напитков - Электрические чайники",
          "url_handle": "/category/tehnika-dlja-kuhni/prigotovlenie-napitkov/jelektricheskie-chajniki?recommended_by=listing"
        }
      ],
      "discount_percent": 33,
      ...
      "url_handle": "/product/chaynik-tefal-thermo-protect-ko-140ae0-286429?recommended_by=listing",
      "discount": 33,
  ...
  "categories": [
    {
      ...
      "url": "/category/smartfony-i-gadzhety/aksessuary-dlja-telefonov?recommended_by=listing",
      "url_handle": "/category/smartfony-i-gadzhety/aksessuary-dlja-telefonov?recommended_by=listing",
      "count": 220
    }
  ],
  "locations": [
...
|------------------------------------------|----------|------------------------------------------------------------------------------------------------------|
| html                                     | String   | Сгенерированный HTML для списка товаров из шаблона в личном кабинете                                 |
...
| categories[].url_handle                  | String   | Ссылка на категорию без домена. Например: `/categories/smartphones`                                  |
