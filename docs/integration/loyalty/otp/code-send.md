...
Параметр `event` может быть любой строкой из латинских букв в нижнем регистре, цифр, и `_`, которым мы отделяем разные проверки. Для одного номера можно параллельно запускать разные проверки. Например:
